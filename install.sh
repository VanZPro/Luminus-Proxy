#!/usr/bin/env bash
# Luminus Pool installer (PRIVATE) for Linux and macOS (also WSL).
# Luminus Pool installer for Linux and macOS (also WSL).
#
...
#
# One-command install:
#   curl -fsSL https://raw.githubusercontent.com/priyo000/luminus/main/install.sh | bash
#
# Or after cloning:
#   bash install.sh
#
# Environment variables (all optional):
#   LUMINUS_HOME       Install directory (default: ~/luminus-pool)
#   LUMINUS_REPO       Repo URL (default: github.com/priyo000/luminus — PRIVATE)
#   LUMINUS_YES=1      Skip confirmation prompts (for CI / unattended installs)
#   LUMINUS_BRANCH     Branch to clone (default: main)
#   LUMINUS_NO_CLI=1   Skip the ~/.local/bin/luminus symlink
#   LUMINUS_SKIP_BROWSERS=1  Skip Playwright/Camoufox download (needed for auth bot)

set -euo pipefail

REPO_URL="${LUMINUS_REPO:-git@github.com:priyo000/luminus.git}"
INSTALL_DIR_DEFAULT="${LUMINUS_HOME:-$HOME/luminus-pool}"
BRANCH="${LUMINUS_BRANCH:-main}"
ASSUME_YES="${LUMINUS_YES:-0}"
...
detect_os() {
  case "$(uname -s)" in
    ...
    FreeBSD*|OpenBSD*|NetBSD*)
      err "BSD detected. Luminus Pool is tested on Linux/macOS/Windows only — proceed at your own risk."
...
show_summary() {
  printf "\n${C_BOLD}${C_BLUE}Luminus-Proxy${C_RESET} — Unified AI provider proxy\n\n"
  ...
  ((total_size += 150))
  if [[ "${LUMINUS_SKIP_BROWSERS:-0}" != "1" ]]; then
  ...
  if [[ "$ASSUME_YES" == "1" ]]; then
    printf "${C_DIM}LUMINUS_YES=1 set — skipping confirmation.${C_RESET}\n\n"
    return
  fi

  # If stdin isn't a tty (e.g. piped from curl), default to yes — otherwise the
  # script just hangs on `read`. Users opt out by exporting LUMINUS_YES=0 and
...
clone_or_update_repo() {
  if [[ -f "package.json" ]] && grep -q '"name": *"luminus-pool"' package.json 2>/dev/null; then
    PROJECT_DIR="$(pwd)"
...
install_cli_symlink() {
  if [[ "${LUMINUS_NO_CLI:-0}" == "1" ]]; then
    warn "LUMINUS_NO_CLI=1 — skipping CLI symlink"
    return
  fi
  step "Installing CLI commands"
  local target="$HOME/.local/bin"
  mkdir -p "$target"
  ln -sf "$PROJECT_DIR/luminus" "$target/luminus"
  chmod +x "$PROJECT_DIR/luminus"
  ok "Linked $target/luminus -> $PROJECT_DIR/luminus"
...
main() {
  printf "\n${C_BOLD}${C_BLUE}Luminus Pool Installer${C_RESET}  ${C_DIM}(%s%s)${C_RESET}\n" "$OS" "${DISTRO_FAMILY:+/$DISTRO_FAMILY}"
  ...
  cd "$PROJECT_DIR"
  chmod +x luminus 2>/dev/null || true
  ...
  printf "Luminus Pool is installed at: ${C_BOLD}%s${C_RESET}\n\n" "$PROJECT_DIR"

  cat <<EOF
${C_BOLD}Quick Start:${C_RESET}

  1. Start the server:
     ${C_CYAN}luminus start${C_RESET}
     ${C_DIM}(or: cd $PROJECT_DIR && ./luminus start)${C_RESET}
...
${C_BOLD}Useful Commands:${C_RESET}
  ...
  luminus status     Check server status
  luminus logs       Tail server logs
  luminus stop       Stop the server
  luminus restart    Restart the server
  luminus doctor     Diagnose installation health
  luminus update     Pull latest, rebuild, restart
  luminus help       Full command reference
...
${C_BOLD}Files of interest:${C_RESET}
  ...
  $PROJECT_DIR/.luminus.log      Server logs

${C_DIM}Tip: re-run this installer any time to pull updates and rebuild.${C_RESET}
${C_DIM}Tip: trouble? run \`luminus doctor\` to get a checklist of fixes.${C_RESET}
EOF
}
