...
import { Plus, Upload, RefreshCw, Play, RotateCcw, Flame, ChevronDown, Loader2, Key, Pencil, Trash2, Zap, Lock, Shield, Eye, EyeOff, Download, ListChecks } from "lucide-react";
...
type Provider = "kiro" | "kiro-pro" | "codebuddy" | "codebuddy-china" | "canva" | "codex" | "qoder" | "gitlab-duo" | "youmind" | "grok-cli" | "blackbox";
...
const providers: Provider[] = ["kiro", "kiro-pro", "codebuddy", "codebuddy-china", "canva", "codex", "qoder", "gitlab-duo", "youmind", "grok-cli", "blackbox"];
...
function labelProvider(provider: string) {
  ...
  if (provider === "grok-cli") return "Grok CLI (OAuth)";
...
export default function Accounts() {
  ...
  const [codebuddyGlobalApiKey, setCodebuddyGlobalApiKey] = useState("");
  const [codebuddyGlobalBulkApiKeys, setCodebuddyGlobalBulkApiKeys] = useState("");
  const [codebuddyGlobalBusy, setCodebuddyGlobalBusy] = useState(false);
  ...
  const [blackboxBulkApiKeys, setBlackboxBulkApiKeys] = useState("");
  const [blackboxBusy, setBlackboxBusy] = useState(false);
  ...
  const [importingByokModels, setImportingByokModels] = useState(false);
  const [byokImportPickerOpen, setByokImportPickerOpen] = useState(false);
  const [byokImportCandidates, setByokImportCandidates] = useState<string[]>([]);
  const [byokImportSelected, setByokImportSelected] = useState<Set<string>>(new Set());
  ...
  async function handleBlackboxApiKey() {
    const apiKey = blackboxApiKey.trim();
    if (!apiKey) { showError(new Error("Paste Blackbox API key")); return; }
    setBlackboxBusy(true);
    try {
      await fetchApi("/api/accounts", {
        method: "POST",
        body: JSON.stringify({ provider: "blackbox", apiKey }),
      });
      showSuccess("Blackbox account added.");
      setAddDialogProvider(null);
      setBlackboxApiKey("");
      await load();
    } catch (e: any) {
      ...
      setBlackboxBusy(false);
    }
  }

  async function handleBlackboxBulkApiKey() {
    const keysText = blackboxBulkApiKeys.trim();
    if (!keysText) { showError(new Error("Paste Blackbox API keys")); return; }
    ...
    try {
      const res = await fetchApi("/api/accounts", {
        method: "POST",
        body: JSON.stringify({ provider: "blackbox", apiKeys: keysText }),
      });
      showSuccess(`Added ${res.count || lines.length} Blackbox accounts.`);
      setAddDialogProvider(null);
      setBlackboxBulkApiKeys("");
      await load();
    } catch (e: any) {
      showError(e);
    } finally {
  ...
  async function handleCodeBuddyGlobalApiKey() {
    const apiKey = codebuddyGlobalApiKey.trim();
    if (!apiKey) { showError(new Error("Paste CodeBuddy API key")); return; }
    setCodebuddyGlobalBusy(true);
    try {
      const res = await fetchApi<any>("/api/accounts", {
        method: "POST",
        body: JSON.stringify({
      ...
      const labelText = res?.email || "account";
      showSuccess(res?.updated
        ? `CodeBuddy key updated (${labelText})`
        : `CodeBuddy ${labelText} added successfully`);
      setCodebuddyGlobalApiKey("");
      setAddDialogProvider(null);
      await load();
    } catch (err) { showError(err); }
    finally { setCodebuddyGlobalBusy(false); }
  }

  async function handleCodeBuddyGlobalBulkApiKey() {
    const keysText = codebuddyGlobalBulkApiKeys.trim();
  ...
  async function handleImportByokModels() {
    if (!byokForm.base_url) {
      showError(new Error("Base URL is required to import models"));
      return;
    }

    // Find the first key that is not the placeholder and not empty
    const firstKeyRow = byokForm.keys.find(k => k.key && k.key !== BYOK_KEY_PLACEHOLDER);
    const apiKey = firstKeyRow ? firstKeyRow.key.trim() : "";
    if (!apiKey) {
      showError(new Error("Please enter a valid API key first to authenticate the import request"));
      return;
    }

    setImportingByokModels(true);
    try {
      ...
      setByokImportCandidates(modelIds);
      // Auto-select all by default
      setByokImportSelected(new Set(modelIds));
      setByokImportPickerOpen(true);
    } catch (err) {
      showError(err);
    } finally {
      setImportingByokModels(false);
    }
  }

  function handleConfirmImportedModels() {
    const selected = Array.from(byokImportSelected);
    if (selected.length === 0) {
      showError(new Error("Please select at least one model to import"));
      return;
    }
    // Merge or overwrite models text area
    const existing = byokForm.models.split(",").map(m => m.trim()).filter(Boolean);
    const combined = Array.from(new Set([...existing, ...selected]));
    setByokForm({
      ...byokForm,
      models: combined.join(", "),
    });
    setByokImportPickerOpen(false);
    showSuccess(`Imported ${selected.length} model(s)`);
  }
  ...
  return (
    <div className="space-y-6">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Button
                    ...
                    onClick={handleImportByokModels}
                    disabled={importingByokModels}
      ...
      {/* BYOK Model Import Picker Dialog */}
      <Dialog open={byokImportPickerOpen} onOpenChange={setByokImportPickerOpen}>
          ...
          <div className="space-y-4 pt-2">
            <div className="flex items-center justify-between border-b border-[var(--border)] pb-2">
              <span className="text-xs text-[var(--muted-foreground)] font-medium">
                {byokImportCandidates.length} models found
              </span>
              <div className="flex gap-2">
                <Button
                  ...
                  onClick={() => setByokImportSelected(new Set(byokImportCandidates))}
                >
                  Select All
                </Button>
                <Button
                  ...
                  onClick={() => setByokImportSelected(new Set())}
                >
                  Clear All
                </Button>
              </div>
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
              {byokImportCandidates.map((modelId) => {
                const isSelected = byokImportSelected.has(modelId);
                return (
                  <label
                    key={modelId}
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        const next = new Set(byokImportSelected);
                        if (next.has(modelId)) {
                          next.delete(modelId);
                        } else {
                          next.add(modelId);
                        }
                        setByokImportSelected(next);
                      }}
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-[var(--border)]">
              <Button
                variant="outline"
                onClick={() => setByokImportPickerOpen(false)}
                className="text-[var(--muted-foreground)]"
              >
                Cancel
              </Button>
              <Button onClick={handleConfirmImportedModels} className="shadow-sm">
                Confirm Import
              </Button>
            </div>
          </div>
      ...
      <Dialog open={addDialogProvider !== null} onOpenChange={(open) => {
        ...
        <DialogContent className="max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {addDialogProvider === "kiro-pro" || addDialogProvider === "codex"
                ...
                ? "Paste CodeBuddy Global API key. Format follows 9router authModes apikey support."
                : addDialogProvider === "codebuddy-china"
                ? "Paste CodeBuddy China API keys (ck_...). Satu key per baris untuk bulk import."
                : addDialogProvider === "blackbox"
                ? "Paste Blackbox API keys. Format supports single or bulk import."
          ...
          {addMode === "pat" && addDialogProvider === "grok-cli" && (
            <div className="space-y-4">
              <p className="text-sm text-[var(--muted-foreground)]">Grok CLI adalah koneksi OAuth native dari 9router, bukan BYOK. Selesaikan OAuth di 9router, lalu jalankan Import 9router untuk memasukkan koneksi terenkripsi ke akun Luminus.</p>
              <div className="flex justify-end gap-2">
                <Button variant="outline" onClick={() => setAddDialogProvider(null)}>Cancel</Button>
                <Button onClick={() => window.open("http://localhost:20128/dashboard/providers/grok-cli", "_blank", "noopener,noreferrer")}>Open 9router OAuth</Button>
              </div>
            </div>
          )}
          ...
          {addMode === "apikey" && addDialogProvider === "codebuddy" && (
            <div className="space-y-4">
              <div className="rounded-md border border-[var(--border)] bg-[var(--secondary)]/30 p-3 text-sm text-[var(--muted-foreground)]">
                <p className="font-medium text-[var(--foreground)]">Bulk API keys</p>
                <p>One key per line. Format: <code>name|apiKey</code> or just <code>apiKey</code> (auto-named by index).</p>
              </div>
              <div>
                <label className="text-sm text-[var(--foreground)]">CodeBuddy Global API Keys</label>
                <textarea
                  value={codebuddyGlobalBulkApiKeys}
                  onChange={(e) => setCodebuddyGlobalBulkApiKeys(e.target.value)}
                  className="mt-1 w-full h-40 rounded-md border border-[var(--border)] bg-[var(--background)] p-3 text-sm font-mono text-[var(--foreground)] placeholder:text-[var(--muted-foreground)] focus:outline-none focus:ring-1 focus:ring-[var(--ring)] resize-none"
                  placeholder="Ares BUDY|sk-...\nsk-..."
                  disabled={codebuddyGlobalBusy}
                />
                <p className="mt-1 text-xs text-[var(--muted-foreground)]">
                  Supports models: <code>cb-opus-4.8</code>, <code>cb-gpt-5.1</code>, <code>cb-gemini-2.5-pro</code>, etc.
                </p>
              </div>
              <details className="rounded-md border border-[var(--border)] p-3">
                <summary className="cursor-pointer text-sm font-medium text-[var(--foreground)]">Single API key</summary>
                <div className="mt-3 space-y-3">
                  <div>
                    <label className="text-sm text-[var(--foreground)]">API Key</label>
                    <Input
                      value={codebuddyGlobalApiKey}
                      onChange={(e) => setCodebuddyGlobalApiKey(e.target.value)}
                      placeholder="Paste one CodeBuddy API key here..."
                      className="mt-1 font-mono text-sm"
                      disabled={codebuddyGlobalBusy}
                    />
                  </div>
                  <Button onClick={handleCodeBuddyGlobalApiKey} disabled={codebuddyGlobalBusy || !codebuddyGlobalApiKey.trim()}>
                    {codebuddyGlobalBusy ? (<><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Adding...</>) : "Add Single Account"}
                  </Button>
                </div>
              </details>
              <div className="flex justify-end gap-2">
                <Button variant="outline" onClick={() => setAddDialogProvider(null)} disabled={codebuddyGlobalBusy}>Cancel</Button>
                <Button onClick={handleCodeBuddyGlobalBulkApiKey} disabled={codebuddyGlobalBusy || !codebuddyGlobalBulkApiKeys.trim()}>
                  {codebuddyGlobalBusy ? (<><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Importing...</>) : "Add Accounts"}
                </Button>
              </div>
            </div>
          )}
...
ck_xyz789ghi012..."
          ...
          {addMode === "apikey" && addDialogProvider === "blackbox" && (
            <div className="space-y-4">
              <div className="rounded-md border border-[var(--border)] bg-[var(--secondary)]/30 p-3 text-sm text-[var(--muted-foreground)]">
                <p className="font-medium text-[var(--foreground)]">Bulk Blackbox API keys</p>
                <p>One key per line. Format: <code>name|apiKey</code> or just <code>apiKey</code>.</p>
              </div>
              <div>
                <label className="text-sm text-[var(--foreground)]">Blackbox Global API Keys</label>
                <textarea
                  value={blackboxBulkApiKeys}
                  onChange={(e) => setBlackboxBulkApiKeys(e.target.value)}
                  className="mt-1 w-full h-40 rounded-md border border-[var(--border)] bg-[var(--background)] p-3 text-sm font-mono text-[var(--foreground)] placeholder:text-[var(--muted-foreground)] focus:outline-none focus:ring-1 focus:ring-[var(--ring)] resize-none"
                  placeholder="lybra|bb-...\nbb-..."
                  disabled={blackboxBusy}
                />
                <p className="mt-1 text-xs text-[var(--muted-foreground)]">
                  Supports models: <code>bb/claude-sonnet-4.6</code>, <code>bb/gpt-5.5</code>, <code>bb/deepseek-v4-flash</code>, etc.
                </p>
              </div>
              <details className="rounded-md border border-[var(--border)] p-3">
                <summary className="cursor-pointer text-sm font-medium text-[var(--foreground)]">Single API key</summary>
                <div className="mt-3 space-y-3">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setBlackboxApiKey(e.target.value)}
                      placeholder="Paste one Blackbox API key here..."
                      className="mt-1 font-mono text-sm"
                      disabled={blackboxBusy}
                    />
                  </div>
                  <Button onClick={handleBlackboxApiKey} disabled={blackboxBusy || !blackboxApiKey.trim()}>
                    {blackboxBusy ? (<><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Adding...</>) : "Add Single Account"}
                  </Button>
                </div>
              </details>
              <div className="flex justify-end gap-2">
                <Button variant="outline" onClick={() => setAddDialogProvider(null)} disabled={blackboxBusy}>Cancel</Button>
                <Button onClick={handleBlackboxBulkApiKey} disabled={blackboxBusy || !blackboxBulkApiKeys.trim()}>
                  {blackboxBusy ? (<><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Importing...</>) : "Add Accounts"}
                </Button>
              </div>
            </div>
          )}
