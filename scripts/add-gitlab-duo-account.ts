/**
 * Add a GitLab Duo account using the running luminus API.
 *
 * Usage (env or args):
...
const LUMINUS_PORT = process.env.LUMINUS_PORT ?? "1930";
const LUMINUS_HOST = process.env.LUMINUS_HOST ?? "127.0.0.1";
const LUMINUS_URL = `http://${LUMINUS_HOST}:${LUMINUS_PORT}`;
const LUMINUS_API_KEY = process.env.LUMINUS_API_KEY ?? process.env.API_KEY ?? "pool-proxy-secret-key";

console.log(`[req] POST ${LUMINUS_URL}/api/accounts/gitlab-duo  base=${baseUrl} pat-len=${pat.length}`);

const res = await fetch(`${LUMINUS_URL}/api/accounts/gitlab-duo`, {
  method: "POST",
  headers: {
    "Content-Type": "application/json",
    Authorization: `Bearer ${LUMINUS_API_KEY}`,
  },
