#!/usr/bin/env bun
/**
 * Luminus Pool — Client Integration CLI
 *
 * Detects installed AI coding clients and configures them to use the
 * luminus-pool proxy. Run from any machine; only needs the proxy URL and API key.
 *
 * Usage:
...
async function restoreAllBackups() {
  ...
  for (const clientId of Object.keys(CLIENT_META) as ClientTarget[]) {
    ...
    const backups = files
      .filter((f) =>
        f.startsWith(path.basename(configPath) + ".luminus-backup-")
      )
      .sort()
      .reverse();
...
async function interactiveMode(info: ProxyConnectionInfo) {
  ...
  console.log("\nDone! Your tools should now route through luminus-pool.\n");
}
...
Detects installed AI coding clients and configures them to use the
luminus-pool proxy. Sets proxy URL, API key, and default model.
...
`);
  ...
  if (!opts.dryRun) {
    console.log("Done! Your tools should now route through luminus-pool.");
