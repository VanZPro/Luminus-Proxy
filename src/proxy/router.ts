  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    try {
      ...
      if (
        ...
        if (refreshResult.success && 'tokens' in refreshResult && refreshResult.tokens) {
          // Parse tokens string to store as jsonb
          let parsedTokens: unknown;
          try {
            parsedTokens = JSON.parse('tokens' in refreshResult ? refreshResult.tokens : "{}");
          } catch {
            parsedTokens = 'tokens' in refreshResult ? refreshResult.tokens : null;
          }
