...
function cloneLogBody(value: unknown): unknown {
  if (Array.isArray(value)) return value.map(cloneLogBody);
  if (value && typeof value === "object") {
    const out: Record<string, unknown> = {};
    for (const [key, val] of Object.entries(value as Record<string, unknown>)) {
      out[key] = cloneLogBody(val);
    }
    return out;
  }
  return value;
...
export function prepareLogBody(value: unknown): unknown {
  ...
  if (logBodyFull) return cloneLogBody(value);

  const redacted = logBodyRedact ? redactLogBody(value) : cloneLogBody(value);
