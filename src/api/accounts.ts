...
accountsRouter.get("/byok", async (c) => {
  ...
  // Preset providers: shown even when no keys are imported yet so users can
  // configure them from the dashboard without first importing keys from 9router.
  const PRESETS: Array<{ prefix: string; base_url: string; models: string[] }> = [
    {
      prefix: "blackbox",
  ...
  // Seed empty preset groups so they appear in the dashboard with 0 keys.
  for (const preset of PRESETS) {
    if (!byokAccounts.some((acc) => getByokPrefix(acc) === preset.prefix)) {
      groups.set(preset.prefix, {
        id: 0,
        label: preset.prefix,
        base_url: preset.base_url,
        format: "openai",
        models: preset.models,
        model_prefix: preset.prefix,
        headers: {},
        status: "active",
        enabled: true,
        available_models: preset.models.map((m) => `${preset.prefix}-${m}`),
        key_count: 0,
        active_key_count: 0,
...
accountsRouter.post("/byok/:id/test", async (c) => {
  ...
  // Note: model is not strictly validated against tokens.models — the proxy
  // accepts any model id (matches 9router behavior). We only warn if a model
  // was provided that isn't in the configured list, but still attempt the test.
  const isRegisteredModel = !reqBody.model || tokens.models.includes(reqBody.model);
  const modelWarning = !isRegisteredModel
    ? `Model "${reqBody.model}" is not yet saved in the provider's model list — test will still attempt to route it upstream. Save the provider to register it.`
    : undefined;
 ...
 * Fetch the /models endpoint of a remote OpenAI-compatible provider using
 * the supplied base_url + api_key, returning a list of model IDs. Used by
 * the BYOK dialog "Import from /models" button so the user does not have
 * to type model IDs manually.
 *
 * Body: { base_url: string, api_key: string, headers?: Record<string,string> }
 * Returns: { models: Array<{ id: string; owned_by?: string }>, count: number }
 *
 * Security notes:
 *   - This is a one-shot proxy fetch — no credentials are stored.
 *   - The api_key supplied here is NOT persisted to the database. The
 *     caller must still save the BYOK provider separately via POST /byok.
 *   - The base_url is normalised (trailing slash trimmed) and the path
 *     "/models" is appended if not already present.
 */
accountsRouter.post("/byok/import-models", async (c) => {
  ...
  // Build the models URL. If the caller already included /models, use as-is.
  ...
  try {
    ...
    if (response.status === 401 || response.status === 403) {
      return c.json({
        ...
        error: `Upstream rejected the provider API key (HTTP ${response.status}). This must be the provider/BYOK key, not a Luminus dashboard key.`,
        upstream_url: modelsUrl,
        latency_ms: latencyMs,
      }, 401);
    }
    if (!response.ok) {
      ...
      return c.json({
        ...
        error: `Upstream ${modelsUrl} returned HTTP ${response.status}: ${text.slice(0, 300)}`,
        upstream_url: modelsUrl,
        latency_ms: latencyMs,
      }, 502);
    }
    if (!contentType.toLowerCase().includes("json")) {
      ...
      return c.json({
        ...
        error: `The URL returned ${contentType || "non-JSON content"}, not an OpenAI-compatible model list. You may have entered the Luminus dashboard URL (1931) instead of the API URL (1930), or omitted /v1.`,
        upstream_url: modelsUrl,
    ...
    type RemoteModel = { id?: string; owned_by?: string };
    ...
    const raw: RemoteModel[] = Array.isArray(json)
      ? json
      : (json && "data" in json ? json.data : null) ??
        (json && "models" in json ? json.models : null) ??
        [];

    const models = raw
      .map((m) => ({ id: (m?.id || "").toString(), owned_by: m?.owned_by || undefined }))
      .filter((m) => m.id);
...
export async function createGitlabDuoAccount(
  ...
  const headers = {
    "Private-Token": pat,
    "Content-Type": "application/json",
    "User-Agent": "luminus-pool/gitlab-duo",
...
accountsRouter.post("/", async (c) => {
  const body = await c.req.json<{
    provider: "kiro" | "kiro-pro" | "codebuddy" | "codebuddy-china" | "canva" | "codex" | "qoder" | "gitlab-duo" | "youmind" | "blackbox";
    email?: string;
    password?: string;
  ...
  // ── CodeBuddy Global: Bulk API key flow ──────────────────────────────
  // Accept multiple API keys (one per line, format: name|apiKey or just apiKey),
  // validate format, and create account per key with auto-generated email label.
  if (body.provider === "codebuddy" && body.apiKeys) {
    const lines = body.apiKeys
      .split("\n")
      .map((k: string) => k.trim())
      .filter((k: string) => k.length > 0);
    ...
    // Parse format: name|apiKey or just apiKey
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (parts.length >= 2) {
        // Format: name|apiKey
        keys.push({ name: parts[0]!.trim(), apiKey: parts[1]!.trim() });
      } else {
        // Format: just apiKey (auto-name by index)
        keys.push({ name: `key-${i + 1}`, apiKey: line });
      }
    }

    const created: Array<{ id: number; email: string }> = [];

    for (const { name, apiKey } of keys) {
      if (!apiKey) continue;

      const email = `cb-${name}-${crypto.createHash("sha256").update(apiKey).digest("hex").slice(0, 8)}`;
      const encryptedKey = encrypt(apiKey);

      // Store API key in BOTH password (for encryption) and tokens (for provider to read)
      const tokens = JSON.stringify({ api_key: apiKey });
  ...
  // ── CodeBuddy Global: API key flow ────────────────────────────────
  // Accept single API key (CodeBuddy global), validate, and create account
  // with auto-generated email label. Mirrors YouMind flow.
  if (body.provider === "codebuddy" && body.apiKey) {
    const trimmed = body.apiKey.trim();
    if (!trimmed) return c.json({ error: "apiKey is empty" }, 400);

    // Generate stable email label from API key hash
    const email = `cb-global-${crypto.createHash("sha256").update(trimmed).digest("hex").slice(0, 12)}`;
    const encryptedKey = encrypt(trimmed);

    // Store API key in tokens for provider to read
    const tokens = JSON.stringify({ api_key: trimmed });

    // Upsert by (provider, email) to allow key rotation
    const existing = await db.select().from(accounts)
      .where(eq(accounts.email, email))
  ...
  // ── Blackbox: Bulk API key flow ─────────────────────────────────────
  // Accept multiple API keys (one per line, format: name|apiKey or just apiKey),
  // validate format, and create account per key with auto-generated email label.
  if (body.provider === "blackbox" && body.apiKeys) {
    const lines = body.apiKeys
      .split("\n")
      .map((k: string) => k.trim())
      .filter((k: string) => k.length > 0);
    ...
    for (const { name, apiKey } of keys) {
      ...
      const email = `bb-${name}-${crypto.createHash("sha256").update(apiKey).digest("hex").slice(0, 8)}`;
      const encryptedKey = encrypt(apiKey);
    ...
    pool.invalidate("blackbox");
    broadcast({ type: "account_created", data: { provider: "blackbox", count: created.length } });
  ...
  // ── Blackbox: Single API key flow ──────────────────────────────────
  if (body.provider === "blackbox" && body.apiKey) {
    const trimmed = body.apiKey.trim();
    if (!trimmed) return c.json({ error: "apiKey is empty" }, 400);

    const email = `bb-global-${crypto.createHash("sha256").update(trimmed).digest("hex").slice(0, 12)}`;
    const encryptedKey = encrypt(trimmed);
    const tokens = JSON.stringify({ api_key: trimmed });

    const existing = await db.select().from(accounts)
      .where(eq(accounts.email, email))
      .then((rows) => rows.find((r) => r.provider === "blackbox"));
    ...
    if (existing) {
      ...
      pool.invalidate("blackbox");
      broadcast({ type: "account_updated", data: { id: existing.id, provider: "blackbox", status: "active" } });
      return c.json({ id: existing.id, provider: "blackbox", email, status: "active", updated: true }, 200);
    }
    ...
    const created = inserted[0]!;
    pool.invalidate("blackbox");
    broadcast({ type: "account_created", data: { id: created.id, provider: "blackbox", email } });
    return c.json({ ...created, password: "***", tokens: "[set]" }, 201);
  }
