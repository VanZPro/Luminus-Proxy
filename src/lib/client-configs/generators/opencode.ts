...
export async function configureOpenCode(
  ...
  try {
    // Preview mode: generate a clean luminus-only config
    ...
    config.model = `luminus/${defaultModel}`;
    if (typeof config.small_model !== "string" || config.small_model.startsWith("luminus/")) {
      config.small_model = `luminus/${defaultModel}`;
    }
    if (
      Array.isArray(config.enabled_providers) &&
      !config.enabled_providers.includes("luminus")
    ) {
      config.enabled_providers = [...(config.enabled_providers as string[]), "luminus"];
    }
