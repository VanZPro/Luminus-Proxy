...
function upsertCodexConfig(content: string, info: ProxyConnectionInfo): string {
  ...
  const withProvider = upsertRootTomlString(
    upsertRootTomlString(content, "model_provider", "luminus"),
    "model",
    info.modelId
  );
  const withoutLuminus = removeTomlSection(
    removeTomlSection(withProvider, "model_providers.luminus"),
    'model_providers."luminus"'
  );
  const separator = withoutLuminus.trim() ? `${newline}${newline}` : "";
  return `${withoutLuminus.trimEnd()}${separator}[model_providers.luminus]${newline}name = "Luminus Pool"${newline}base_url = "${escapeTomlString(
    info.openaiBaseUrl
