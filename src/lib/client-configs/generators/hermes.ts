...
export async function configureHermes(
  ...
  try {
    ...
    const kiroProviderRegex = /^\s*- name:\s*luminus\b[\s\S]*?(?=^\s*- name:|^[a-z]|$)/gm;
    ...
    const modelSection = `model:${newline}  default: "luminus/${info.modelId}"${newline}  provider: "luminus"${newline}`;
