declare module "hono" {
  interface ContextVariableMap {
    apiKeyRow: import("./api/keys").ResolvedApiKey["row"];
  }
}
...
import { isValidApiKey, resolveApiKey, isManagementAllowed } from "./api/keys";
...
app.use("/v1/*", async (c, next) => {
  ...
  // Resolve custom API key row (null = legacy global key, no per-key restrictions)
...
// API Key authentication for management API — applies to /api/* including /api/auth/*
app.use("/api/*", async (c, next) => {
  ...
  // Public-only keys cannot access the management API — they're proxy-only.
  const resolved = await resolveApiKey(token);
  if (!isManagementAllowed(resolved.row)) {
    return c.json(
      {
        error: {
          message: "This API key is restricted to public proxy use only and cannot access the management API.",
          type: "forbidden",
...
setInterval(() => {
  if (globalThis.Bun && typeof Bun.gc === "function") {
    Bun.gc(true); // force synchronous GC to keep RAM footprint low
  }
}, 60000); // 1 minute
