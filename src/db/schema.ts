...
// Custom API keys with per-key permissions. The old global `settings.api_key`
// continues to work as the env/config fallback; once any row exists here,
// requests may use any of these keys (subject to per-key model allowlist + quota).
export const apiKeys = sqliteTable("api_keys", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  name: text("name").notNull(), // user-friendly label, e.g. "sk-bansos"
  key: text("key").notNull().unique(), // bearer token
  allowedModels: text("allowed_models", { mode: "json" }).$defaultFn(() => [] as string[]),
  /** JSON map: model id -> per-model max total tokens over the lifetime of this key. */
  modelTokenLimits: text("model_token_limits", { mode: "json" }).$defaultFn(() => ({} as Record<string, number>)),
  /** JSON map: model id -> tokens consumed so far (mirrors request_logs). */
  modelTokensUsed: text("model_tokens_used", { mode: "json" }).$defaultFn(() => ({} as Record<string, number>)),
  totalTokenLimit: integer("total_token_limit").default(0), // 0 = unlimited
  totalTokensUsed: integer("total_tokens_used").default(0),
  enabled: integer("enabled", { mode: "boolean" }).notNull().default(true),
  isPublicOnly: integer("is_public_only", { mode: "boolean" }).notNull().default(true),
